...
public class LayerCape implements LayerRenderer
{
    ...
    public void doRenderLayer(AbstractClientPlayer entitylivingbaseIn, float p_177141_2_, float p_177141_3_, float partialTicks, float p_177141_5_, float p_177141_6_, float p_177141_7_, float scale)
    {
    	if (entitylivingbaseIn.hasPlayerInfo() && !entitylivingbaseIn.isInvisible() && entitylivingbaseIn.isWearing(EnumPlayerModelParts.CAPE) && entitylivingbaseIn.getName().equals(Minecraft.getMinecraft().getSession().getUsername()))
        {
            ...
            if(Minecraft.getMinecraft().getSession().getUsername() == "Smitty_007")
            	this.playerRenderer.bindTexture(new ResourceLocation("pf/cape/cape.png"));
            else if(Minecraft.getMinecraft().getSession().getUsername() == "Mysuperconstruct")
            	this.playerRenderer.bindTexture(new ResourceLocation("pf/cape/cape-m.png"));
            else if(Minecraft.getMinecraft().getSession().getUsername() == "LeNinjaHD")
            	this.playerRenderer.bindTexture(new ResourceLocation("pf/cape/cape-p.png"));
            else if(Minecraft.getMinecraft().getSession().getUsername() == "DragonKingTv")
            	this.playerRenderer.bindTexture(new ResourceLocation("pf/cape/cape-d.png"));
            else 
            	this.playerRenderer.bindTexture(new ResourceLocation("pf/cape/cape-d.png"));
